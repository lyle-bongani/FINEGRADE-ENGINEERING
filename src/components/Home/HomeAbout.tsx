    ...
    return (
        <section className="w-full flex justify-start py-20 bg-transparent relative">
            ...
            <img
                ...
                style={{ filter: 'invert(38%) sepia(99%) saturate(7492%) hue-rotate(2deg) brightness(104%) contrast(105%)', maxWidth: '90vw', transform: 'rotate(180deg) scaleX(-1)' }}
            />
            {/* Large faint quote mark as background accent */}
            <span className="absolute left-0 top-0 text-[16rem] font-extrabold text-orange-100 z-0 select-none pointer-events-none leading-none">"</span>
            <div className="max-w-3xl w-full relative z-20 pl-32 pr-8">
                <h2 className="text-4xl md:text-5xl font-extrabold text-orange-500 mb-2 mt-2 text-left">About Finegrade Safety & Equipment</h2>
                <div className="text-orange-400 text-lg font-semibold mb-6 text-left">Safety is not just a policy—it's our culture.</div>
                ...
                <div className="text-left">
                    <Link to="/about" className="mt-8 px-5 py-2 border-2 border-orange-500 text-orange-500 font-bold rounded-full shadow hover:bg-orange-500 hover:text-white transition-colors text-base inline-block bg-transparent">Learn More</Link>
                </div>
            </div>
        </section>
    );
